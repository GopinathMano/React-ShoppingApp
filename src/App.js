...
function App() {
  const [items] = useState([
    { name: "samsang mobie", price: "40$", addedToCart: false },
    { name: "xiaomi mobile", price: "60$", addedToCart: false },
    { name: "oneplus", price: "80$", addedToCart: false },
    { name: "Apple", price: "100$", addedToCart: false }
  ]);
  ...
  const callBackSet = (data, name) => {
    ...
    console.log(name);
    items.map((prod) => {
      if (name === prod.name) prod.addedToCart = !prod.addedToCart;
      return prod;
    });
    console.log(items);
  };
